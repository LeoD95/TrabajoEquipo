...
$("#frmAlta").submit(function(e){
    ...
    // validacion para no meter id de medicamento en 0
    if(idMedicamento==0){
        ...
        .setting({
            'title':'Información',
            'label':'Salir',
            'message': 'Debes seleccionar el medicamento.' ,
            'onok': function(){ alertify.message('Gracias !');}
        }).show();
        return false;       
    }
     // validacion para no meter id de farmacia en 0
     if(idFarmacia==0){
        alertify.dialog('alert').set({transition:'zoom',message: 'Transition effect: zoom'}).show();
        ...
        .setting({
            'title':'Información',
            'label':'Salir',
            'message': 'Debes seleccionar la farmacia.' ,
            'onok': function(){ alertify.message('Gracias !');}
        }).show();
        return false;       
    }
        ...
        $.ajax({
            ...
            alertify.success('Se ha guardado el pedido' );
            $("#frmAlta")[0].reset();
            llenar_persona();
            },
            error:function(xhr,status){
